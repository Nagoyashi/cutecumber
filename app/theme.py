...
Shape v1 (stored in users.theme_json):
    {
      "version": 1,
      "preset": "<one of PRESETS>",
      "overrides": { <subset of COLOR_KEYS + ENUM_KEYS, diffs from preset only> }
    }
...
ENUM_KEYS = {
    ...
    "decoration": ("none", "sparkles", "hearts", "stars"),
}
...
def _decoration_layer(kind: str, accent: str) -> str:
    path = _DECOR_PATHS[kind]
    svg = (
...
def _page_background(t: dict) -> str:
    ...
    if t["decoration"] != "none":
        layers.append(_decoration_layer(t["decoration"], t["accent"]))
